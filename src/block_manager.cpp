...
void BlockManager::ValidateBlockId(block_id_t block_id) const {
    if (block_id == INVALID_BLOCK_ID) {
        const auto info = std::unordered_map<std::string, std::string>{
            {"block_id", std::to_string(block_id)}
        };
        throw duckdb::InvalidInputException("Block ID cannot be INVALID_BLOCK_ID", info);
    }
    if (block_id < 0) {
        const auto info = std::unordered_map<std::string, std::string>{
            {"block_id", std::to_string(block_id)}
        };
        throw duckdb::InvalidInputException("Block ID cannot be negative", info);
    }
    if (block_id >= max_block) {
        ...
        throw duckdb::InvalidInputException("Block ID cannot exceed max_block", info);
    }
}
