...
namespace
{
    ...
    void callback_set_cache_size(duckdb::ClientContext& context, duckdb::SetScope scope, duckdb::Value& value)
    {
        ...
        auto state_ptr = quackstore::ExtensionState::RetrieveFromContext(context);
        if (!state_ptr) {
...
ExtensionParams ExtensionParams::ReadFrom(duckdb::optional_ptr<duckdb::FileOpener> opener) {
    ...
    duckdb::Value value;
    if (duckdb::FileOpener::TryGetCurrentSetting(opener, PARAM_NAME_QUACKSTORE_CACHE_ENABLED, value)) {
        auto enabled = value.GetValue<bool>();
        result.cache_enabled = enabled;
    }
    if (duckdb::FileOpener::TryGetCurrentSetting(opener, PARAM_NAME_QUACKSTORE_CACHE_SIZE, value)) {
        auto cache_size_in_bytes = value.GetValue<uint64_t>();
        result.max_cache_size = cache_size_in_bytes;
    }
    if (duckdb::FileOpener::TryGetCurrentSetting(opener, PARAM_NAME_QUACKSTORE_CACHE_PATH, value)) {
        auto path = value.GetValue<duckdb::string>();
        result.cache_path = path;
    }
    if (duckdb::FileOpener::TryGetCurrentSetting(opener, PARAM_NAME_QUACKSTORE_DATA_MUTABLE, value)) {
...
ExtensionParams ExtensionParams::ReadFrom(const duckdb::ClientContext& context) {
    ...
    duckdb::Value value;
    if (context.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_ENABLED, value)) {
        auto enabled = value.GetValue<bool>();
        result.cache_enabled = enabled;
    }
    if (context.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_SIZE, value)) {
        auto cache_size_in_bytes = value.GetValue<uint64_t>();
        result.max_cache_size = cache_size_in_bytes;
    }
    if (context.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_PATH, value)) {
        auto path = value.GetValue<duckdb::string>();
        result.cache_path = path;
    }
    if (context.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_DATA_MUTABLE, value)) {
        auto data_mutable = value.GetValue<bool>();
        result.data_mutable = data_mutable;
    }
...
ExtensionParams ExtensionParams::ReadFrom(const duckdb::DatabaseInstance& instance) {
    ...
    duckdb::Value value;
    if (instance.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_ENABLED, value)) {
        auto enabled = value.GetValue<bool>();
        result.cache_enabled = enabled;
    }
    if (instance.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_SIZE, value)) {
        auto cache_size_in_bytes = value.GetValue<uint64_t>();
        result.max_cache_size = cache_size_in_bytes;
    }
    if (instance.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_CACHE_PATH, value)) {
        auto path = value.GetValue<duckdb::string>();
        result.cache_path = path;
    }
    if (instance.TryGetCurrentSetting(PARAM_NAME_QUACKSTORE_DATA_MUTABLE, value)) {
        auto data_mutable = value.GetValue<bool>();
        result.data_mutable = data_mutable;
    }
...
void ExtensionParams::AddExtensionOptions(duckdb::DBConfig &config) {
    ...
    config.AddExtensionOption(
        PARAM_NAME_QUACKSTORE_CACHE_ENABLED, 
    ...
    config.AddExtensionOption(
        PARAM_NAME_QUACKSTORE_CACHE_SIZE, 
        "Cache size (bytes)",
    ...
    config.AddExtensionOption(
        PARAM_NAME_QUACKSTORE_CACHE_PATH, 
        "Cache path",
    ...
    config.AddExtensionOption(
        PARAM_NAME_QUACKSTORE_DATA_MUTABLE, 
